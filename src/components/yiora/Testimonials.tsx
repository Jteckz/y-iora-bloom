...
export function Testimonials() {
  return (
    <Section id="voices" tone="linen" divider="wavy" dividerPosition="top">
      <div
        aria-hidden
      ...
      <Reveal className="mx-auto max-w-2xl text-center">
        ...
        <h2 className="mt-4 font-display text-[clamp(2rem,4.6vw,3.2rem)] leading-[0.98] font-[650] tracking-[-0.03em] text-cocoa text-balance">
          What the room <span className="italic font-normal text-cocoa/60">sounds like</span>{" "}
          afterwards.
        </h2>
      ...
      <div className="mt-12 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {VOICES.map((v, i) => (
          <Reveal as="figure" key={v.name} delay={i * 70}>
            ...
            >
              <span aria-hidden className="font-display text-[2.2rem] leading-none text-rose/25">
                “
              </span>
              ...
              <figcaption className="mt-6 flex items-center gap-3 border-t border-cocoa/8 pt-4">
                ...
                <span className="min-w-0">
                  <span className="block text-[0.9rem] font-semibold tracking-[-0.01em] text-cocoa truncate">
                    {v.name}
                  </span>
                  <span className="block text-[0.72rem] leading-tight text-cocoa/55 truncate">
                    {v.role}
                  </span>
                </span>
              </figcaption>
              ...
              <span
                aria-hidden
                className="pointer-events-none absolute right-4 top-4 h-6 w-6 rounded-full border border-cocoa/5 opacity-0 transition-opacity duration-200 group-hover:opacity-100"
              />
            </div>
          </Reveal>
        ))}
